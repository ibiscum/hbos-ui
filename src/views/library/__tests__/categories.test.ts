...
describe('categories.vue', () => {
    ...
    const router = createRouter({
      ...
      routes: [
        ...
        },
        {
          ...
          component: { template: '<div>Category Albums</div>' },
        },
      ],
    })
  ...
  it('loads categories on mount and renders fetched items', async () => {
  ...
  it('calls getAvailableLibrary before categories request', async () => {
    const { wrapper } = await createWrapper()
    await flushPromises()
  ...
  it('shows loading skeleton while categories request is pending', async () => {
  ...
  it('navigates to albums-by-category when a category card is clicked', async () => {
    ...
    expect(router.currentRoute.value.name).toBe('albums-by-category')
    expect(router.currentRoute.value.params.category).toBe('Rock')
  })

  it('renders empty-state copy when API returns no categories', async () => {
    ...
    expect(wrapper.text()).toContain('No categories found. Add category mappings to create categories.')
  })
  ...
  it('shows API error toast and leaves empty state visible', async () => {
    ...
    expect(showErrorToastMock).toHaveBeenCalledWith(
      'Failed to load categories: backend unavailable',
    )
    expect(wrapper.findAll('.poster-item')).toHaveLength(0)
    expect(wrapper.text()).toContain('No categories found. Add category mappings to create categories.')
  })
  ...
  it('normalizes non-string API errors', async () => {
    ...
    expect(showErrorToastMock).toHaveBeenCalledWith('Failed to load categories: Unknown error')
  })
  ...
  it('handles thrown errors and clears loading state', async () => {
    ...
    expect(showErrorToastMock).toHaveBeenCalledWith('An error occurred while loading categories')
    expect(wrapper.find('[data-testid="poster-skeleton"]').exists()).toBe(false)
    expect(wrapper.text()).toContain('No categories found. Add category mappings to create categories.')
  })
