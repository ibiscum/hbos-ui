...
describe('BluetoothSettings.vue', () => {
  ...
  describe('Modal Functionality', () => {
    it('should show modal when backend returns true', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({
          data: { capability: 'KeyboardOnly', discoverable: true },
      ...
      mockFetch.mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ modal: true }),
      } as Response)

      // Advance timer to trigger modal check
      vi.advanceTimersByTime(1000)
      await flushPromises()

      // Modal should be shown (we would need to check v-model state)
      expect(mockFetch).toHaveBeenCalledWith('http://api.test/bluetooth/modal')
    })

    it('should handle string true response from modal API', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({
          data: { capability: 'KeyboardOnly', discoverable: true },
      ...
      vi.clearAllMocks()
      // Backend returns string "true" instead of boolean
      mockFetch.mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({ modal: 'true' }),
      } as Response)
    ...
    it('should not show modal when backend returns false', async () => {
      mockFetch.mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({
          data: { capability: 'KeyboardOnly', discoverable: true },
    ...
    it('should handle modal API errors gracefully', async () => {
      ...
      // Should not throw - continues polling
      expect(wrapper.exists()).toBe(true)
    })
  })
